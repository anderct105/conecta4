...
public class IU_Menu extends Stage implements Observer {
	...
	public void listenerIdiomas() {
		idi_es.addEventHandler(MouseEvent.MOUSE_CLICKED, event -> {
			GestorIdiomas.getmGestorIdiomas().setIdioma(0);
			idioma();
		});
		idi_eu.addEventHandler(MouseEvent.MOUSE_CLICKED, event -> {
	...
	public void jugar() {
		...
		play.setDisable(true);
		config.setDisable(true);
		ft.setOnFinished(event -> cambiarATablero());
		ft.play();
	}
	...
	public void setModoJuego() {
		...
		if (GestorIdiomas.getmGestorIdiomas().getIdioma(0).get("ordenador_facil").equals(modoJuego)) {
			modo.setText((String)frases.get("modo") + GestorIdiomas.getmGestorIdiomas().getIdiomaActual().get("ordenador_facil"));
		} else if (GestorIdiomas.getmGestorIdiomas().getIdioma(0).get("ordenador_dificil").equals(modoJuego)) {
			modo.setText((String)frases.get("modo") + GestorIdiomas.getmGestorIdiomas().getIdiomaActual().get("ordenador_dificil"));
		} else {
